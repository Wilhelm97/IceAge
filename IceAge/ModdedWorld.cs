...
namespace IceAge
{
    public class ModdedWorld : ModWorld
    {
        public static int biomeTiles = 200;
        ...
        public override void ModifyWorldGenTasks(List<GenPass> tasks, ref float totalWeight)
        {
            ...
            if(ShiniesIndex != -1)
            {
                tasks.Insert(ShiniesIndex + 1, new PassLegacy("Ice Mod Ores", delegate (GenerationProgress progress)
                {
                    ...
                    for(int i = 0; i < (int)((double)(Main.maxTilesX * Main.maxTilesY) * 6E-05); i++)
                    {
                        WorldGen.TileRunner(
                            ...
                            mod.TileType("OreTile"), // The tile type that will be spawned
                            false, // Add Tile ???
                            0f, // Speed X ???
                            0f, // Speed Y ???
                            false, // noYChange ??? 
            ...
            if(GraniteIndex != -1)
            {
                tasks.Insert(GraniteIndex + 1, new PassLegacy("Ice Basic Biome", delegate (GenerationProgress progress)
                {
        ...
        {
            biomeTiles = tileCounts[mod.TileType("IceBiomeTile")];
        }
